...
- ⚙️ 取消常驻 WebDAV 配置表单：云端备份卡片右上角新增小配置按钮，点击弹出 WebDAV 配置对话框；未配置时点备份/恢复会自动唤起配置弹窗
- 🔄 云端/本地恢复共用同一校验流程（解包校验 → 兜底备份 → 落位替换 → 自动重启），坏包不会损坏现有数据
- ⏻ 悬浮窗面板标题行新增关闭按钮（⏻）：一键关闭悬浮球并同步关闭设置页开关，开机/应用自启不再复活；✕ 仍为收起面板
- 🧹 123 网盘分发目录只保留 APK，清理 wbl-update.json / SHA256SUMS.txt 冗余文件；publish.py 不再上传

## v1.5.1 (10501) — 重复任务 + 稍后再提醒

- 🔁 重复任务：Task.repeat（不重复/每天/每周/每月，仅在设置了提醒时间时生效）；完成重复任务自动把提醒滚动到下一周期并重排闹钟（TaskRepo.setDone 统一入口，应用内/小组件/悬浮球/通知行为一致）
- ⏭️ 过期补滚：RepeatRule.next 纯函数——过期周期一次性补滚到下一个未来时点；每月按日历加月、月末夹取（1/31→2/28）；13 项 JUnit 单测全过
- 😴 稍后再提醒：提醒通知新增 [完成][5分钟后再提醒][10分钟后再提醒] 三按钮；SNOOZE 广播改期 remind_at 并重排闹钟、reminded 清零
- 🗄️ DB v3 迁移：tasks 表新增 repeat 列（INTEGER DEFAULT 0），老数据无损升级
- 列表/小组件显示重复徽标（🔁每天 📅每周 🗓️每月）

### 真机验证（realme RMX3888, ColorOS/Android 16）
- 每天滚动：remind +24h 秒级精确，dumpsys alarm origWhen 与 DB 完全吻合
- 每周滚动：now-3d 完成 → +7d = now+4d 精确值吻合；每月滚动：单次广播精确 +1 月（毫秒级保留）
- snooze 5min：remind_at = 广播时刻+5min，闹钟重排吻合；普通任务对照 done=1 不受滚动影响
- 提醒通知真实触发（任务过期弹通知），三按钮 action 齐全；置顶通知「下一个提醒」同步刷新
- realme freezer 冻结场景：排队广播解冻后依次执行，每次滚动一个周期，语义正确
- DB v3 迁移后原 10 条任务无损，App 冷启动无崩溃

### 发布记录
- release APK 10,036,137B，sha256 1b9946c64322d65cc264c6591cfb121557decd06c1d90642e6f980fc62aa2898
- 同一 debug keystore 签名，覆盖升级数据保留

## v1.5.0 (10500) — 桌面小组件

- 🧩 桌面小组件（4×2 可调大小）：长按桌面 → 卡片 → 搜索「忘不了」添加；桌面直接查看待办清单，排序与应用内一致（置顶 > 紧急 > 提醒时间 > 创建时间）
- ✅ 快速勾选：点任务左侧圆圈即标记已办，小组件即时刷新，置顶通知/保活通知同步更新，无需打开应用
- ➕ 快速新增：点小组件右上角「+」直达新建任务编辑页（EXTRA_NEW_TASK 深链）
- 🔄 实时联动：应用内新增/编辑/删除/勾选任务后小组件自动刷新（TaskRepo 钩子）；开机、恢复数据、App 启动时兜底刷新
- 空态提示「暂无待办 · 点右上角 + 记一笔」；header 显示「N 条待办」计数与全部完成状态

### 真机验证（realme RMX3888, ColorOS/Android 16）
- 添加小组件到桌面：通过（appwidget list 确认 id=2, host=com.android.launcher）
- 待办列表渲染 + 计数：通过（2 条待办正确显示，勾选后 2→1→0）
- 勾选切换落库：通过（tasks.done=1，updated_at 更新，widget 行消失）
- 应用内新增任务 → 小组件实时刷新：通过（TaskRepo.save 钩子生效）
- 「+」深链按钮：代码已验证（PendingIntent 指向 MainActivity EXTRA_NEW_TASK），真机点击验证因设备使用中跳过

### 发布记录
- release APK 10,032,173B，sha256 32842907b8352a66655d5f1ba8ca22ba0b062cc7ceb2ab16da776bed39ababca
- 同一 debug keystore 签名，覆盖升级数据保留

## v1.4.1 (10401) — 数据备份恢复 · 崩溃监控 · 更新线路加固

- ☁️ WebDAV 数据备份/恢复：设置页新增「数据备份」卡片，服务器地址/账号/密码仅存本机（零硬编码凭据），一键备份任务数据库 + 照片主题；恢复前自动先备份当前数据到 files/pre_restore/（后悔药），恢复后自动重启生效；E2E 实测 PUT 备份/GET 恢复/数据回滚全链路通过
- 🧯 崩溃监控 CrashReporter：未捕获异常落盘 files/crash/（时间戳+版本+设备+堆栈，只留最近 5 份），下次启动弹窗「上次运行发生异常」+ 一键复制反馈；已配置 WebDAV 时检查更新顺带匿名上报（成功后删本地，失败保留重试）；只记录堆栈与设备/版本信息，不采集任务内容
- 🔀 OTA 线路重排：官网 Pages 直链第一位、GitHub 直连次位、gh-proxy/ghproxy.net 镜像居中、api.github 资产兜底、ghfast.top 置末；检查线路 5/5、下载线路 5/6（ghfast 整站暂不可达，客户端自动切换）
- 🔏 发布签名统一：release 签名指向标准 Android debug 证书（各机可复现），覆盖升级不再因签名差异被拒；真机覆盖安装实测数据完整保留
- 🧪 对比度逻辑固化：新增 util/ColorContrast.kt 纯函数（WCAG 相对亮度/对比度/渐变最差对比度/黑白字选择/veil 补偿，零 UI 依赖），ThemeUi 顶栏逻辑改为委托；JUnit 五类场景 21 用例全通过（含极光青/鎏金/樱花粉/午夜蓝真实主题渐变回归）；真机 chk.py 列表页对比度 FAILS:0
- 📦 工程加固：真机数据快照归档 release/snapshots/；junit4.12+hamcrest1.3 固化进本地 Maven 仓库；新增 app/src/test 单测源集
...
- 🧭 排版大改版：底部导航栏（待办 / 日历 / 主题 / 设置），页面切换更顺手；编辑页保持全屏专注输入
- 🖼️ 主题独立成页：十套主题 + 自定义照片主题从设置页独立为底部导航「主题」页，预览更大、切换更直接；设置页专注功能开关
